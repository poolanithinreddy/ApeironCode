...
export class SandboxManager {
  ...
  async getAvailableRunner(): Promise<SandboxRunner | 'local'> {
    ...
    // `APEIRONCODE_TEST_OFFLINE` is preferred; `OPENCODE_TEST_OFFLINE` is kept as
    // a deprecated legacy alias for existing CI/automation.
    const testOffline =
      process.env['APEIRONCODE_TEST_OFFLINE'] === '1' || process.env['OPENCODE_TEST_OFFLINE'] === '1';
