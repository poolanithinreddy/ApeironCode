...
describe('buildChatRequestBody (OpenAI-compatible tool schemas)', () => {
  it('sanitizes tools for OpenAI, not only GitHub Models', () => {
    ...
    // `revert_patch` wraps its object in `.refine(...)` (a ZodEffects). The
    // maintained converter now surfaces its real optional properties; the old
    // hand-rolled converter incorrectly dropped them to an empty object.
    expect(params).toEqual([
      {
        type: 'object',
        properties: {
          editId: {type: 'string', minLength: 1},
