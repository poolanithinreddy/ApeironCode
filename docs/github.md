...
- Automation workflows are dry-run by default. Non-dry-run writes require `OPENCODE_AUTOMATION=1` plus the narrow action flag, such as `OPENCODE_AUTOMATION_REVIEW=1`, `OPENCODE_AUTOMATION_COMMENT=1`, `OPENCODE_AUTOMATION_COMMIT=1`, or `OPENCODE_AUTOMATION_PR_CREATE=1`.
- Automation can also be constrained with `OPENCODE_AUTOMATION_REPOS`, `OPENCODE_AUTOMATION_ACTORS`, and `OPENCODE_AUTOMATION_DENY_ACTORS`. Explicit denies win.
